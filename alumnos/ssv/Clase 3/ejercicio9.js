...
const pandemicCode = (code) => {
  const codeLength = code.length;
  const numberOfX = code.split('').filter((item) => item === 'X').length;
  const worldPopulation = codeLength - numberOfX;
  const codeArray = code.split('X');
  let infected = 0;
  for (let continent of codeArray) {
    if (continent.includes('1')) {
      infected += continent.length;
    }
  }
  return codeLength !== numberOfX ? (infected * 100) / worldPopulation : 0;
};
